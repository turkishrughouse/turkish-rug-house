...
type ProductTagSource = "title" | "category" | "subcategory" | "manual"
type ProductTagItem = {
    value: string
...
function parseManualTagItems(value: string | null | undefined): ProductTagItem[] {
    return (value || "")
        .split(",")
        .map((item) => normalizeTagValue(item))
        .filter(Boolean)
        .filter((item, index, array) => array.findIndex((candidate) => normalizeTagKey(candidate) === normalizeTagKey(item)) === index)
        .map((item) => ({
            value: item,
            autoGenerated: false,
...
function joinTagValues(tags: ProductTagItem[]) {
    return tags.map((tag) => tag.value).join(", ")
}

function syncAutoTag(
    tags: ProductTagItem[],
    source: Exclude<ProductTagSource, "manual">,
    nextValue: string,
    blockedSources: Record<string, boolean>
) {
    const normalizedNextValue = normalizeTagValue(nextValue)
    const withoutSource = tags.filter((tag) => !(tag.autoGenerated && tag.source === source))
    if (!normalizedNextValue || blockedSources[source]) return withoutSource

    const nextKey = normalizeTagKey(normalizedNextValue)
    const duplicate = withoutSource.find((tag) => normalizeTagKey(tag.value) === nextKey)
    if (duplicate) return withoutSource
...
function RichTextEditor({
    ...
    const ensureTableCellPlaceholders = (root: HTMLElement) => {
        // Make empty cells editable across browsers by keeping a <br> placeholder.
        // This prevents the “cell becomes non-editable after deleting content” issue.
    ...
    const normalizeForVisualMount = (raw: string) => {
        const normalized = normalizeRichTextHtml(raw || "")
        // Ensure tables remain editable after Visual<->Code switches.
        const wrapper = document.createElement("div")
        wrapper.innerHTML = normalized
        ensureTableCellPlaceholders(wrapper)
        return wrapper.innerHTML
    }

    const emitChange = (nextValue: string) => {
        // IMPORTANT: do NOT normalize on every keystroke.
        // Live normalization can delete empty table cells/blocks and breaks native undo/caret behavior.
    ...
    const saveSelection = () => {
        ...
        if (!selection || selection.rangeCount === 0) {
            // Selection can be lost after destructive edits (e.g. deleting a table) or
            // when focus moves to dialogs. Clear stale ranges so future inserts fall back safely.
        ...
        if (
            ...
            (range.commonAncestorContainer as unknown as { isConnected?: boolean }).isConnected === false ||
            !visualRef.current.contains(range.commonAncestorContainer)
        ) {
            savedRangeRef.current = null
            return
        }
    ...
    const syncContent = () => {
        ...
        // Keep table cells editable if user clears them.
        ensureTableCellPlaceholders(visualRef.current)
    ...
    useEffect(() => {
        ...
        if (!visualRef.current) return
        // Only mount/refresh Visual HTML when we are NOT actively editing.
        // Resetting innerHTML while typing causes:
        // - caret jumps (often to start)
        // - broken editing inside table cells
        // - broken native undo/redo history
        if (isFocusedRef.current || isComposingRef.current) return
        const nextHtml = normalizeForVisualMount(value || "")
    ...
    useEffect(() => {
        // When switching from Code -> Visual, mount sanitized HTML once.
        if (mode !== "visual") return
        if (!visualRef.current) return
        const nextHtml = normalizeForVisualMount(value || "")
        if (visualRef.current.innerHTML !== nextHtml) {
        ...
        // Switching modes resets DOM; selection must be treated as unknown.
        savedRangeRef.current = null
        // Place caret at end if nothing is selected (prevents “dead editor” feeling).
        try {
            const sel = window.getSelection()
            if (sel && sel.rangeCount === 0) {
                const r = document.createRange()
                r.selectNodeContents(visualRef.current)
                r.collapse(false)
                sel.removeAllRanges()
                sel.addRange(r)
            }
    ...
    const insertHtml = (html: string) => {
        ...
        const selection = window.getSelection()
        visualRef.current.focus()
        const isRangeUsable = (candidate: Range | null) => {
            if (!candidate || !visualRef.current) return false
            const start = candidate.startContainer
            const end = candidate.endContainer
            const startConnected = (start as unknown as { isConnected?: boolean }).isConnected !== false
            const endConnected = (end as unknown as { isConnected?: boolean }).isConnected !== false
            if (!startConnected || !endConnected) return false
            return visualRef.current.contains(start) && visualRef.current.contains(end)
        }

        let range: Range | null = null
        if (savedRangeRef.current) {
            const cloned = savedRangeRef.current.cloneRange()
            if (isRangeUsable(cloned)) range = cloned
        ...
        try {
            ...
            // If the saved range became invalid due to DOM mutations, fall back to end of editor.
            range = document.createRange()
            range.selectNodeContents(visualRef.current)
            range.collapse(false)
        }
        ...
        try {
            ...
            // As a fallback, insert via execCommand which is more forgiving in some browsers.
            ...
            {mode === "visual" ? (
                <div
                    ...
                    onBlur={() => {
                        ...
                        syncContent()
                        // After blur, selection is unreliable; clear it.
                        savedRangeRef.current = null
                    }}
...
export function ProductForm({ lang = "en", initialData, options }: ProductFormProps) {
    ...
    const [tags, setTags] = useState<ProductTagItem[]>(() => parseManualTagItems(initialData?.seoKeywords || ""))
    const [blockedAutoTagSources, setBlockedAutoTagSources] = useState<Record<string, boolean>>({})
    const [isKeywordsAutoSync, setIsKeywordsAutoSync] = useState(() => !(initialData?.seoKeywords || "").trim())
    ...
    const defaultValues: Partial<ProductFormValues> = initialData ? {
        ...
        seoDescription: (initialData.seoDescription && !/<(?:p|table|ul|ol|div|span|strong|b|em|i|br)\b/i.test(initialData.seoDescription)) ? initialData.seoDescription : "",
    ...
    const primaryCategoryLineage = useMemo(() => {
        const firstCategoryId = selectedCategoryIds?.[0]
        if (!firstCategoryId) return [] as Category[]
        const lineage: Category[] = []
        let current = categoryLookup.get(firstCategoryId) || null
        while (current) {
            lineage.unshift(current)
            current = current.parentId ? categoryLookup.get(current.parentId) || null : null
        }
        return lineage
    }, [categoryLookup, selectedCategoryIds])
    const mainCategoryTagValue = primaryCategoryLineage[0]?.title || ""
    const subcategoryTagValue = primaryCategoryLineage.length > 1 ? primaryCategoryLineage[primaryCategoryLineage.length - 1]?.title || "" : ""
    const resolvedSeoTitle = (seoTitleValue || title || tx("Product title preview", "Ürün başlığı önizleme")).trim()
    const manualMetaPreview = stripHtmlForSeo(seoDescriptionValue)
    const mainDescPreview = stripHtmlForSeo(descriptionValue)
    const previewDescriptionSource = manualMetaPreview || mainDescPreview || tx("Product description preview", "Ürün açıklaması önizleme")
    ...
    useEffect(() => {
        setTags((current) => {
            let next = current
            next = syncAutoTag(next, "title", title || "", blockedAutoTagSources)
            next = syncAutoTag(next, "category", mainCategoryTagValue, blockedAutoTagSources)
            next = syncAutoTag(next, "subcategory", subcategoryTagValue, blockedAutoTagSources)

            const deduped: ProductTagItem[] = []
            ...
            for (const tag of next) {
                ...
                seen.add(key)
                deduped.push({ ...tag, value: normalizedValue })
            }

            const unchanged =
                deduped.length === current.length &&
                deduped.every((tag, index) =>
                    current[index] &&
                    current[index].value === tag.value &&
                    current[index].autoGenerated === tag.autoGenerated &&
                    current[index].source === tag.source
                )
            return unchanged ? current : deduped
        })
    }, [blockedAutoTagSources, mainCategoryTagValue, subcategoryTagValue, title])

    useEffect(() => {
        if (!isKeywordsAutoSync) return
        const joinedTags = joinTagValues(tags)
        if (seoKeywordsValue === joinedTags) return
        setValue("seoKeywords", joinedTags, { shouldValidate: true, shouldDirty: true, shouldTouch: true })
    }, [isKeywordsAutoSync, seoKeywordsValue, setValue, tags])
    ...
    const addTag = (rawValue: string) => {
        ...
        if (!nextTag) return
        const normalized = normalizeTagValue(nextTag)
        const normalizedKey = normalizeTagKey(normalized)
        setTags((current) => {
            const existingIndex = current.findIndex((item) => normalizeTagKey(item.value) === normalizedKey)
            if (existingIndex >= 0) {
                const existing = current[existingIndex]
                if (!existing.autoGenerated) return current
                return current.map((item, index) =>
                    index === existingIndex
                        ? { value: normalized, autoGenerated: false, source: "manual" as const }
                        : item
                )
            }
            return [...current, { value: normalized, autoGenerated: false, source: "manual" as const }]
        })
        setIsKeywordsAutoSync(true)
        setTagInput("")
    }

    const removeTag = (tagToRemove: string) => {
        setTags((current) => {
            const tag = current.find((item) => item.value === tagToRemove)
            if (tag?.autoGenerated) {
                setBlockedAutoTagSources((prev) => ({ ...prev, [tag.source]: true }))
            }
            return current.filter((item) => item.value !== tagToRemove)
        })
    }
                    ...
                    <aside className="space-y-7 xl:col-start-2">
                        ...
                        <div className="rounded-sm border border-[#c3c4c7] bg-white">
                            ...
                            <div className="space-y-4 p-4">
                                ...
                                <div className="space-y-1">
                                    <Label className="text-xs font-medium uppercase tracking-wide text-slate-600">{tx("Meta description", "Meta açıklama")}</Label>
                                    <textarea
                                        {...register("seoDescription")}
                                        maxLength={160}
                                        className="min-h-[96px] w-full rounded-sm border border-[#8c8f94] px-3 py-2 text-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#2271b1]"
                                        placeholder={tx("SEO description for search engines", "Arama motorları için SEO açıklaması")}
                                    />
                                    <p className="text-xs text-slate-500">
                                        {tx("If left empty, metadata falls back to the main product description.", "Boş bırakılırsa metadata ana ürün açıklamasına geri döner.")}
                                    </p>
                                </div>
                                <div className="space-y-1">
                                    ...
                                    <textarea
                                        ...
                                        onChange={(event) => {
                                            setValue("seoKeywords", event.target.value, { shouldValidate: true, shouldDirty: true, shouldTouch: true })
                                            setIsKeywordsAutoSync(false)
                                        }}
                                ...
                                <p className="rounded-sm border border-[#dcdcde] bg-[#f6f7f7] px-3 py-2 text-xs text-slate-500">
                                    {tx("Search preview uses Meta description first, then falls back to the main product description.", "Arama önizlemesi önce Meta açıklamayı, boşsa ana ürün açıklamasını kullanır.")}
                                </p>
                            </div>
                        </div>
        ...
        {mediaPickerOpen ? (
            <MediaPickerDialog
                ...
                productMeta={{
                    title: title || "",
                    sku: watch("sku") || "",
                    description: stripHtmlForSeo(descriptionValue || ""),
